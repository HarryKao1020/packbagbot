...
NAMING, DIRECTION, COUNTY, TYPE_ONE, TYPE_TWO, TYPE_THREE, TRAFFIC, PLACE_ONE, PLACE_TWO, PLACE_THREE, PLACE_FOUR, PLACE_FIVE = range(12)
...
def start(bot, update):
    ...
    logger.info("username: %s start",update.message.from_user)
    keyboard = [
        [InlineKeyboardButton("北部", callback_data=str(selcountyforN)),
         InlineKeyboardButton("中部", callback_data=str(selcountyforM))],
         [InlineKeyboardButton("南部", callback_data=str(selcountyforS)),
         InlineKeyboardButton("東部", callback_data=str(selcountyforE))]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.message.reply_text('請問這次要去哪裡玩呢？',reply_markup=reply_markup)
    return DIRECTION

def selcountyforN(bot, update):
...
def placeforother(bot, update):
    logger.info("%s prees 自行前往", update.message.from_user)
    reply_keyboard=[['景點一','景點二','景點三','景點四']]
    update.message.reply_text('請選擇你喜歡的景點:',reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
    return PLACE_ONE
#############seconed place

def place_two(bot, update):
    UserID = update.message.from_user['id']
    logger.info("%s prees 自行前往", UserID)
...
def done(bot,update):
    ...
    update.message.reply_text('旅泊包幫你安排好行程嘍，行程如下:')
...
conv_handler = ConversationHandler(
        ...
        states={
            ...
            DIRECTION: [
                        CallbackQueryHandler(selcountyforN, pattern='^' + str(selcountyforN) + '$'),
                        CallbackQueryHandler(selcountyforM, pattern='^' + str(selcountyforM) + '$'),
                        CallbackQueryHandler(selcountyforS, pattern='^' + str(selcountyforS) + '$'),
                        CallbackQueryHandler(selcountyforE, pattern='^' + str(selcountyforE) + '$')
                        ],
            ...
            TRAFFIC:[
                    MessageHandler(Filters.regex('^(開車🚘)$'), placeforcar),
                    MessageHandler(Filters.regex('^(火車🚂)$'), placeforother),
                    MessageHandler(Filters.regex('^(客運🚌)$'), placeforother),
                    MessageHandler(Filters.regex('^(高鐵🚅)$'), placeforother),
            ],
            PLACE_ONE:[CommandHandler('restart', restart),
                ...
                CommandHandler('next', place_two),
                CommandHandler('done', done),
                MessageHandler(Filters.regex('^(下一個)$'), place_two),
                MessageHandler(Filters.regex('^(完成)$'), done)],
            PLACE_TWO:[CommandHandler('restart', restart),
                CallbackQueryHandler(returnbutton, pattern='^(上一頁)$'),
                CallbackQueryHandler(confirmbutton, pattern='^' + str(confirmbutton) + '$'),
                CallbackQueryHandler(placedetail),
                CommandHandler('next', place_two),
